...
class Table {
    ...
    public function find($options) {
        ...
        if (isset($options['bind'])) {
            return $this->bind_query($sql->to_s(), $options, $readonly, $eager_load);
        }
    ...
    public static function getBindType(string $type): int {
        switch ($type) {
    ...
    public function bind_query(string $sql, array $options, bool $readonly = false, $includes = null) {
        $sth = $this->conn->prepare($sql);
        $binds = $options['bind'];
        $btypes = $options['bindTypes'] ?? [];
        foreach ($binds as $key => $value) {
            $bp = $btypes[$key] ?? self::getBindType(gettype($value));
            $sth->bindValue($key, $value, $bp);
        }
        $check = $sth->execute();
        return $this->sth_result($sth, $readonly, $includes);
    }

    public function sth_result($sth, $readonly = false, $includes = null)
    {
        $self = $this;
